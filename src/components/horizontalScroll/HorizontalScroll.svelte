...
<div bind:this={scrollContainer} class={cssClass}>
  <button
    ...
    class="button button--primary button--icon-only horizontal-scroll-nav button-prev"
  >
    <Icon name="angle-left" width="14" />
  </button>
  <button
    on:click={() => horizontalScrollHandler.scrollWithButton(SCROLLDIRECTION.right)}
    class="button button--primary button--icon-only horizontal-scroll-nav button-next"
  >
